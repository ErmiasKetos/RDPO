...
with st.form("po_request_form"):
    ...
    with col1:
        requester = st.text_input("Requester Full Name", required=True)
        link = st.text_input("Link to Item(s)", required=True)
        ...
        attention_to = st.text_input("Attention To", required=True)
    
    with col2:
        department = st.text_input("Department", value="R&D", disabled=True)
        description = st.text_area("Brief Description of Use", required=True)
